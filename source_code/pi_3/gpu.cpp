...
gpu::gpu(u32 vertex_buffer_size, u32 triangle_buffer_size) :
...
constexpr f64 tau = 6.28318530718;
f64 sin(f64 x)
{
    if(x < 0.25 * tau)
        return x / (0.25 * tau);
    else if(x < 0.75 * tau)
        return 2.0 - x / (0.25 * tau);
    else
        return -4.0 + x / (0.25 * tau);
}
f64 cos(f64 x)
{
    if(x < 0.5 * tau)
        return 1.0 - x / (0.25 * tau);
    else
...
void gpu::render(u16 width, u16 height, u32 frame_buffer, vertex* new_vertex_buffer, triangle* new_triangle_buffer, u32 vertex_buffer_size, u32 triangle_buffer_size)
{
    ...
    static f64 angle = 0.0;
    f64 radius = 40.0;

    vertex_buffer[0].x = static_cast<u16>(400.0 + radius * cos(angle)) << 4;
    vertex_buffer[0].y = static_cast<u16>(200.0 + radius * sin(angle)) << 4;
    vertex_buffer[1].x = static_cast<u16>(300.0 + radius * cos(angle)) << 4;
    vertex_buffer[1].y = static_cast<u16>(280.0 + radius * sin(angle)) << 4;
    vertex_buffer[2].x = static_cast<u16>(500.0 + radius * cos(angle)) << 4;
    vertex_buffer[2].y = static_cast<u16>(280.0 + radius * sin(angle)) << 4;
    angle += tau/100.0;
    if(angle > tau)
        angle = 0.0;
