...
CBcmFrameBuffer::CBcmFrameBuffer (unsigned nWidth,
				  unsigned nHeight,
				  unsigned nDepth,
				  unsigned nVirtualWidth,
				  unsigned nVirtualHeight)
...
boolean CBcmFrameBuffer::Initialize (void)
{
	CleanDataCache ();
	DataSyncBarrier ();
	u32 nResult = m_MailBox.WriteRead (GPU_MEM_BASE + (u32) (u64) m_pInfo);
	InvalidateDataCache ();
